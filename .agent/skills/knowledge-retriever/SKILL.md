...
echo "{\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"task\":\"knowledge-retrieval\",\"model\":\"gemini-2.5-pro\",\"duration_sec\":${DURATION}}" >> .claude/logs/gemini_usage.jsonl
```
...
-   **コンテキスト分離の利点**: このワークフローでは、メインエージェントは `index.json`（約88KB）を一切読み込みません。Gemini CLIが別コンテキストで処理するため、メインエージェントのコンテキストウィンドウを100%節約できます。
-   **AI検索の信頼性**: Gemini 2.5 Proは1Mトークンのコンテキストウィンドウを持ち、index.json全体を総合的に分析できます。タグの表記ゆれ（例: "MCP" と "Model Context Protocol"）も適切に認識します。
-   **選択的な参照**: Gemini CLIが選択した記事IDのみを読み込みます。通常3〜5件の記事に絞り込まれるため、必要最小限のコンテキスト消費で済みます。これらの記事はあくまで「こういう手段もあるんだな」という参考情報です。
