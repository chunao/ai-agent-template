...
| **全テスト（`pytest`）が未実行** | **テスト実行を促す（必須）** |
| **ワークフロー定義変更時、関連テスト未実行** | **`.claude/rules/testing.md` の「変更影響マップ」を参照し、関連テストを実行（必須）** |

> **重要**: PR作成前に、必ず全テスト（`pytest`）を実行してください。特に、`.claude/**/*.md` を変更した場合は、testing.mdの「変更影響マップ」を確認し、対応するテストファイルを実行してください。
