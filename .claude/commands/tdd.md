...
- Issueの受け入れ基準を確認
- 正常系・異常系・境界値の観点でテストケースを洗い出す
- 各受け入れ基準に対応するテストを計画
- テスト設計方針（モック/スタブの使用、テストデータ準備）を決定

> **重要**: PLANフェーズを設けることで、REVIEWフェーズでの手戻りを最小化できます

### 2. RED - テストを書く

PLANフェーズで計画したテストを実装してください：

- 計画したテストケースを実装
- テストが失敗することを確認

### 3. REVIEW - テスト項目の妥当性評価
...
- 各フェーズを必ず順番に実行すること（**PLAN → RED → REVIEW → GREEN → REFACTOR**）
- PLANフェーズでテストケースを事前に洗い出すことで、網羅性を確保
